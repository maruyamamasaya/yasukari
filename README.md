...
- `lib/mockUserDb.ts`
  ライト会員登録・ログイン機能のための簡易インメモリDB。デモアカウントと新規登録ユーザーを同じレイヤーで扱います。
- `middleware.ts`
...
ライトプランの体験を素早く試せるよう、フロントエンドのみで完結する仮実装を用意しました。登録完了時には `auth` クッキーを付与し、マイページに自動遷移するためログイン操作を挟む必要がありません。

### フロー概要

1. `/signup` ページでライトプランの説明と入力フォームを表示
2. メールアドレスとパスワード（確認入力付き）をPOST
3. `/api/signup` が仮想DBにユーザーを保存し `auth` クッキーを発行
4. フロントエンドが `/mypage` へ遷移し、ログイン済みの前提で体験を続行

### 仮想DB

- `lib/mockUserDb.ts`
  - メモリ常駐の `Map` を用いてユーザーを保持
  - デモ用アカウント `adminuser` を初期データとして登録
  - `registerLightMember` で重複チェック・登録、`authenticateUser` でログイン判定を実装
- サーバー再起動時に初期化されるため、本番導入時は永続データベースへの移行が必要

### API エンドポイント

- `POST /api/signup`
  - リクエスト : `{ email, password }`
  - 成功時 : 201 を返却し `auth` クッキーを付与
  - 重複時 : 409 を返却しエラーメッセージを通知
- `POST /api/login`
  - 仮想DBに対して認証を実施し、成功時に `auth` クッキーを再発行

### UI ノート

- `/signup` ではライトプランの特徴・登録後の流れをサイドカードで案内
- CTA ボタンは1つに絞り、登録完了後の行き先（マイページ自動遷移）を明示
- 利用規約とプライバシーポリシーへの同意をフッターテキストで提示
