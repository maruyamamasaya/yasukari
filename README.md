...
- **技術スタック**: Next.js + TypeScript。`pages/` を中心に静的生成とAPI Routesを併用し、Tailwind CSSベースのスタイルを `styles/` 配下で管理しています。
- **主要ページ**: トップ、ジャンル別一覧（`/t/genre/*`）、料金・保険・ご利用案内、ニュース（`/news`）、店舗ブログ（`/customer_blog`）、ヘルプ（`/help`）、お問い合わせ（`/contact`）、法定ページ一式を提供しています。
- **UI/UX**: ヒーロースライダー、BikeModelCarouselや人気モデルカルーセルなどのSwiper系コンポーネント、FAQアコーディオン、チャットボット／ウィジェット、最近チェックした商品、モバイル固定ナビなどを実装しています。
- **認証・会員フロー**: `/signup` からのライト会員登録と `/register/auth` の2段階認証、Cognito Hosted UI連携のログイン（`/auth/callback`・`/auth/logout`）、簡易ログイン（`/login`）、モックDBによる会員・待機レコード管理を備えています。
- **コンテンツ生成**: `manual_for_system/` と `blog_for_custmor/` のMarkdown記事を静的生成し、一覧・タグ・ページネーションを提供。`data/` 配下のFAQやバイクデータをチャットボットやカタログで利用しています。
- **運用補助**: `/wait` によるアクセス制御、`lib/` のユーティリティ群（ディレクトリツリー取得、認証コード生成、メール送信モック、レートリミッターなど）、`docs/` の設計メモや更新履歴で運用を支援しています。
