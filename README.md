...
### v0.52
- システムエンジニア・管理者専用サイト更新ブログに10件ずつのページネーションを実装
...
- `pages/manual_for_system/index.tsx`  
  システムエンジニア・管理者専用サイト更新ブログの一覧を生成するファイル。manual_for_systemディレクトリ内のMarkdownを読み込み、日付順に並べ替えて一覧ページを静的生成します。タイトルや抜粋を取得し、リンクをクリックすると詳細ページへ遷移。シンプルなカードデザインで更新日と要約を表示し、サイトの運用状況を把握できます。getStaticPropsを活用することでビルド時にデータを取り込み、表示速度が速い点も特徴です。
