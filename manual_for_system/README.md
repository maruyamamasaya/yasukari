...
- `components/Footer.tsx`  
 ...
 - `pages/index.tsx`
  ホーム画面を構成するメインページ。トップバーでキャンペーンを告知し、ヘッダーには検索バーとナビゲーションを設置。続くヒーローセクションでは背景画像にキャッチコピーとCTAボタンを重ね、特徴を3項目のグリッドで紹介します。その後に新着ブログカルーセル、ジャンル別アイテム、人気モデルカルーセルを配置。Swiperとanimate.cssによるアニメーションを活かし、BikeModelCarouselとFooterコンポーネントを再利用したモダンなトップページです。
- `pages/chat.tsx`  
...
- `pages/manual_for_system/index.tsx`  
  サイト更新ブログの一覧を生成するファイル。manual_for_systemディレクトリ内のMarkdownを読み込み、日付順に並べ替えて一覧ページを静的生成します。タイトルや抜粋を取得し、リンクをクリックすると詳細ページへ遷移。シンプルなカードデザインで更新日と要約を表示し、サイトの運用状況を把握できます。getStaticPropsを活用することでビルド時にデータを取り込み、表示速度が速い点も特徴です。
...
- `manual_for_system/2025-07-20-chatbot-design.md`  
