...
7月1日のアップデートで以下の点を改善しました。

- カテゴリ選択後や自由入力モードから、`&larr; カテゴリ選択に戻る` ボタンでいつでもカテゴリ一覧に戻れるようにしました。
- メッセージ欄の上下にはスクロールボタンを配置し、長い会話でも素早く移動できます。
- `ChatBotWidget` 表示時は高さを `h-[28rem]` に、`/chat` ページでは `h-[80vh]` に拡大し、閲覧しやすくしました。
 さらに `ChatBot` 本体に `resize-y` と `max-h-[150vh]` を付与し、ウィンドウ下部を下にドラッグすると高さを広げ、上にドラッグすると縮める形で、マウスの動きに合わせて調整できるようになりました。
