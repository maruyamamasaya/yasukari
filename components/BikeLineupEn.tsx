  ...
  return (
    <section className="bike-lineup my-5">
      <h2 className="bike-lineup-title">
        BIKE LINE UP
        <span className="bike-lineup-subtitle">Bike Lineup</span>
      </h2>
      <div className="bike-lineup-radio-group">
        {categories.map((c, idx) => (
          <label key={idx} className="bike-lineup-radio">
            ...
            />
            <span
              className="bike-lineup-radio-text"
              dangerouslySetInnerHTML={{ __html: c.label.replace("-", "<br class='sp'>") }}
            />
          </label>
        ))}
      </div>
            ...
            <div key={b.modelCode} className="bike-lineup-card">
              <Link href={`/products/${b.modelCode}`}>
                ...
                <div className="bike-lineup-info">
                  <h3
                    className="bike-lineup-name"
                    dangerouslySetInnerHTML={{ __html: b.modelName.replace(/\\n/g, '<br>') }}
                  />
                </div>
              </Link>
            </div>
