      ...
      <main className="mx-auto flex w-full max-w-5xl flex-col gap-10 px-[5px] py-10">
        <section className="rounded-3xl border border-slate-200 bg-white px-[5px] py-6 shadow-sm sm:px-[5px] sm:py-8">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <Link
              href="/contact"
              className="rounded-full border border-slate-300 px-5 py-2 text-xs font-semibold text-slate-700 transition hover:border-slate-400 hover:text-slate-900"
            >
              お問い合わせ
            </Link>
          </div>
            ...
            {steps.map((step, index) => (
              <div key={step.title}>
                ...
                {index < steps.length - 1 ? (
                  <div className="flex justify-center py-4 text-2xl text-amber-500">
                    ↓
                  </div>
                ) : null}
              </div>
